...
def process_file(mseed_path, plots_directory, catalogue, evid_counter, DEFAULT_MQ_TYPE='impac_mq'):
    freq_min = 0.5
    freq_max = 1.2
...
def main():
    if len(argv) != 2:
        print(f'Usage: {argv[0]} <path to .mseed file or directory with mseed files>')
        return
    ...
    if input_path.is_dir():
        mseed_pathlist = list(input_path.glob('*.mseed'))
        if not mseed_pathlist:
            print(f'No .mseed files found in {input_path}.')
            return
    elif input_path.is_file() and input_path.suffix == '.mseed':
        mseed_pathlist = [input_path]
    else:
        print(f'Error: {input_path} is neither a directory nor a .mseed file.')
        return
    ...
    for mseed_path in mseed_pathlist:
        evid_counter = process_file(
            mseed_path=mseed_path,
            plots_directory=plots_directory,
            catalogue=catalogue,
            evid_counter=evid_counter,
            DEFAULT_MQ_TYPE=DEFAULT_MQ_TYPE
        )
